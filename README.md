...
系统共包含 **35+** 个业务模块，涵盖会员营销系统的完整业务场景。

| 序号 | 模块名称 | 英文标识 | 功能说明 |
|------|---------|---------|---------|
| 1 | 账户模块 | account | 后台账号管理、登录认证、角色权限 |
| 2 | 会员模块 | member | 会员信息、等级、积分管理 |
| 3 | 订单模块 | order | 订单创建、支付、核销、退款 |
| 4 | 商品模块 | goods | 商品信息、分类、库存管理 |
| 5 | 优惠券模块 | coupon | 优惠券发放、使用、核销 |
| 6 | 购物车模块 | cart | 购物车商品管理 |
| 7 | 店铺模块 | store | 店铺信息、配置管理 |
| 8 | 商户模块 | merchant | 商户信息、结算配置 |
| 9 | 地址模块 | address | 收货地址管理 |
| 10 | 售后模块 | aftersale | 售后申请、退款退货 |
| 11 | 预约模块 | appointment | 预约服务管理 |
| 12 | 文章模块 | article | 文章内容管理 |
| 13 | 资产模块 | asset | 用户资产管理 |
| 14 | 余额模块 | balance | 余额充值、消费 |
| 15 | Banner模块 | banner | 轮播图管理 |
| 16 | 卡券模块 | card | 实体卡券管理 |
| 17 | 佣金模块 | commission | 分销佣金管理 |
| 18 | 快递模块 | express | 物流信息管理 |
| 19 | 赠送模块 | give | 礼品赠送管理 |
| 20 | 团购模块 | group | 团购活动管理 |
| 21 | 库存模块 | inventory | 库存管理 |
| 22 | 消息模块 | message | 消息推送管理 |
| 23 | 导航模块 | navigation | 导航菜单管理 |
| 24 | 页面模块 | page | 页面配置管理 |
| 25 | 积分模块 | point | 积分获取、消费 |
| 26 | 充值模块 | recharge | 充值记录管理 |
| 27 | 区域模块 | region | 省市区管理 |
| 28 | 角色模块 | role | 角色权限管理 |
| 29 | 结算模块 | settlement | 商户结算管理 |
| 30 | 短信模块 | sms | 短信发送管理 |
| 31 | 来源模块 | source | 来源渠道管理 |
| 32 | 员工模块 | staff | 员工信息管理 |
| 33 | 订阅模块 | subscription | 消息订阅管理 |
| 34 | 用户模块 | user | 用户信息管理 |
| 35 | 核销模块 | writeoff | 卡券核销管理 |
