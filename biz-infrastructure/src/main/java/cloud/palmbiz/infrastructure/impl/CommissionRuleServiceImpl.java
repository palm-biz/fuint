...
import cloud.palmbiz.infrastructure.mapper.MtCommissionRuleItemMapper;
import cloud.palmbiz.infrastructure.mapper.MtCommissionRuleMapper;
    ...
    public CommissionRuleDto queryCommissionRuleById(Integer id) {
        ...
        if (mtCommissionRuleItems != null && mtCommissionRuleItems.size() > 0) {
            for (MtCommissionRuleItem item : mtCommissionRuleItems) {
                 ...
                 MtGoods mtGoods = goodsQueryService.queryGoodsPOById(item.getTargetId());
                 if (mtGoods != null) {
